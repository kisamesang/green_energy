...
// --- START: กำหนดตัวแปรสำหรับข้อความแจ้งเตือน ---
$error_message = '';
$success_message = '';
...
// --- START: ส่วนการประมวลผล POST (ย้ายกลับมาจาก actions.php) ---
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    
    $action = $_POST['action'] ?? '';

    try {
        // --- ACTION 1: บันทึกการใช้พลังงาน ---
        if ($action === 'log_energy') {
            $kwh_usage = $_POST['kwh_usage'] ?? 0;
            $water_usage = $_POST['water_usage'] ?? 0;
            $log_date = $_POST['log_date'] ?? date('Y-m-d');
            
            // --- START EDIT 1: (NEW) ตรวจสอบการอัปโหลดไฟล์ ---
            if (!isset($_FILES['bill_proof_file']) || $_FILES['bill_proof_file']['error'] != 0) {
                 throw new Exception("เกิดข้อผิดพลาด: กรุณาแนบไฟล์หลักฐานบิลค่าไฟ");
            }
            // --- END EDIT 1 ---
            ...
            // --- START FIX: (MOVED) ย้ายการคำนวณ $month_year มาไว้ก่อน ---
            // คำนวณค่าใช้จ่ายและวันที่เริ่มรอบบิล
            $calculated_cost = $kwh_usage * $kwh_rate;
            $period_start = date('Y-m-01', strtotime($log_date));
            $month_year = date('Y-m', strtotime($log_date));
            // --- END FIX ---
            
            // --- START EDIT 2: (NEW) ประมวลผลไฟล์ที่อัปโหลด ---
            ...
            $file_info = $_FILES['bill_proof_file'];
            ...
            if (!in_array($file_ext, $allowed_exts)) {
                throw new Exception("อนุญาตเฉพาะไฟล์ PDF, PNG, JPG/JPEG เท่านั้น");
            }
            ...
            // --- START FIX: (REMOVED) ลบบรรทัดที่ซ้ำซ้อน (ที่เราย้ายไปแล้ว) ---
            /* (ลบ 4 บรรทัดนี้ออก)
            // คำนวณค่าใช้จ่ายและวันที่เริ่มรอบบิล
            $calculated_cost = $kwh_usage * $kwh_rate;
            $period_start = date('Y-m-01', strtotime($log_date));
            ...
            */
            ...
            if ($result_check->num_rows > 0) {
                // (ถ้าซ้ำ ควรสั่งลบไฟล์ที่เพิ่งอัปโหลด)
                unlink($target_file_path);
                throw new Exception('คุณได้บันทึกข้อมูลสำหรับเดือนนี้ไปแล้ว');
            }

            // --- START EDIT 3: (CHANGED) อัปเดต SQL INSERT ---
            // (เพิ่มคอลัมน์ el_bill_proof_file และ el_verification_status)
            ...
            if ($conn->query($sql_insert) === TRUE) {
                $success_message = 'บันทึกข้อมูลและไฟล์แนบเรียบร้อยแล้ว (รอการตรวจสอบ)';
            } else {
                // (ถ้าบันทึก DB ไม่ผ่าน ควรสั่งลบไฟล์ที่เพิ่งอัปโหลด)
                unlink($target_file_path);
        ...
        // --- ACTION 2: ตั้งเป้าหมายการประหยัด (คงเดิม) ---
            ...
            // แปลง YYYY-MM ให้ตรงกับฐานข้อมูล
            ...
            if ($conn->query($sql_goal) === TRUE) {
                ...
                // (สำคัญ: สั่งให้หน้า Reload ไปยังเดือนที่เพิ่งตั้งค่า)
...
// --- END: ส่วนการประมวลผล POST ---


// 5. ดึงข้อมูล (GET) มาแสดงผล (สำหรับ Summary Cards และ Chart)
// (ส่วนนี้คงเดิมทั้งหมด)

// 5.1 ดึง "ค่าใช้จ่ายทั้งหมด" และ "ลด CO2" (สะสม)
$total_kwh = 0;
$total_cost = 0;
// --- START EDIT 4: (CHANGED) ดึงข้อมูลเฉพาะที่ 'verified' ---
...
// 5.2 (ใหม่) ดึง "ค่าใช้จ่ายที่ประหยัด" (สะสม)
$total_saved_cost = 0;
// --- START EDIT 5: (CHANGED) ดึงข้อมูลเฉพาะที่ 'verified' ---
...
// --- START EDIT 6: (CHANGED) ดึงข้อมูลเฉพาะที่ 'verified' ---
...
// 5.4.2: หาเป้าหมายของ "เดือนที่เลือก"
...
// 5.4.3: หาการใช้งานของ "เดือนที่เลือก"
// --- START EDIT 7: (CHANGED) ดึงข้อมูลเฉพาะที่ 'verified' ---
...
<body class="d-flex flex-column min-vh-100">
    ...
    <main class="main-content">
        <div class="container">
            ...
            <!-- (ส่วน Summary Cards คงเดิม) -->
            <h2 class="h4 mb-3">สรุปผลกระทบ (Overall Impact)</h2>
            <div class="row g-4 mb-4">
                
                <!-- Card 1: ค่าใช้จ่ายที่ "ประหยัด" (ใหม่) -->
                ...
                <!-- Card 2: ค่าใช้จ่าย "ทั้งหมด" (เดิม) -->
                <div class="col-lg-4 col-md-6">
            ...
            <!-- (ส่วนแสดงข้อความ Error/Success คงเดิม) -->
            <!-- --- START EDIT 1: (CHANGED) เพิ่มปุ่มปิด (btn-close) และ class ที่จำเป็น --- -->
            ...
            <div class="row g-4">
                ...
                <div class="col-lg-5">
                    ...
                    <div class="card shadow-sm border-0 mb-4">
                        ...
                        <div class="card-body">
                            <!-- --- START EDIT 8: (CHANGED) เพิ่ม enctype สำหรับการอัปโหลดไฟล์ --- -->
                            ...
                            <!-- --- END EDIT 8 --- -->
                                ...
                                <!-- --- START EDIT 9: (NEW) เพิ่มช่องอัปโหลดไฟล์ --- -->
                                <div class="mb-3">
                                    <label for="bill_proof_file" class="form-label">แนบหลักฐานบิลค่าไฟ (PDF, PNG, JPG)</label>
                                    <input type="file" class="form-control" id="bill_proof_file" name="bill_proof_file" required accept=".pdf,.png,.jpg,.jpeg">
                                    <div class="form-text">ไฟล์ขนาดไม่เกิน 2MB</div>
                                </div>
                                <!-- --- END EDIT 9 --- -->
                    ...
                    <!-- ฟอร์ม 2: ตั้งเป้าหมาย -->
                    <div class="card shadow-sm border-0">
                        <div class="card-header bg-warning text-dark">
                            <i class="fas fa-bullseye card-header-icon"></i>
                            <!-- (3.5.4.6 ข้อ 4) -->
                            ตั้งเป้าหมายการประหยัด 
                        </div>
                        <div class="card-body">
                             <!-- (action ชี้กลับมาที่ไฟล์นี้) -->
    ...
    <!-- JS Libraries (ตามเกณฑ์) -->
    <!-- (สมมติว่ามีไฟล์ Local หรืออยู่ในเครื่องกรรมการ) -->
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // --- START EDIT 2: (NEW) โค้ดสำหรับซ่อน Alert อัตโนมัติ ---
            try {
                // เลือก Alert ทั้งหมดที่มี class .alert-dismissible
                const autoFadeAlerts = document.querySelectorAll('.alert-dismissible');
                
                autoFadeAlerts.forEach(function(alert) {
                    // สร้าง Bootstrap Alert instance (เพื่อให้สั่ง .close() ได้)
            ...
            // (JavaScript สำหรับตั้งค่า default date คงเดิม)
            try {
